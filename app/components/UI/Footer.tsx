...
export default function Footer() {
  ...
  const footerLinks = {
    ...
    connect: [
      { label: "LinkedIn", href: "https://www.linkedin.com/in/raviitsme/" },
  ...
  return (
    <footer className="relative w-full min-h-[85vh] bg-transparent flex flex-col justify-between pt-24 pb-8 px-6 sm:px-12 md:px-24 font-sans antialiased overflow-hidden select-none">
      ...
      >
        ...
        >
          "The next algorithm you understand could change the way you solve
          every problem after it."
        </motion.p>
        ...
        >
          ...
          <a
            href="#"
            className="group relative px-8 py-3.5 rounded-full overflow-hidden bg-zinc-100 text-zinc-950 font-bold tracking-wide transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 shadow-[0_10px_30px_rgba(255,255,255,0.05)]"
          >
          ...
          <a
            href="#"
            className="group relative px-8 py-3.5 rounded-full overflow-hidden bg-zinc-900/40 border border-zinc-800 text-zinc-300 font-medium tracking-wide transition-all duration-300 hover:-translate-y-0.5 hover:border-zinc-700 hover:text-white active:translate-y-0 backdrop-blur-sm"
          >
            Explore Algorithms
          </a>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 sm:gap-8 py-16 text-sm font-light">
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-zinc-500 text-xs leading-relaxed max-w-60">
              Interactive algorithm visualization platform designed to transform
              cold syntax into fluid intuition.
            </p>
          </div>
            ...
            <ul className="flex flex-col gap-2.5 text-zinc-400 text-xs">
              {footerLinks.platform.map((link, idx) => (
                <li key={idx}>
                  <a
                    href={link.href}
                    className="group relative inline-block transition-colors duration-200 hover:text-zinc-100"
                  >
                    {link.label}
          ...
          <div className="flex items-center gap-2 flex-wrap text-zinc-600">
            ...
            {["Next.js", "React", "Tailwind CSS", "Framer Motion"].map(
              (tech, idx) => (
                <span
                  key={idx}
                  className="text-zinc-400 hover:text-cyan-400/80 transition-colors duration-200 cursor-default"
                >
                  {tech}
